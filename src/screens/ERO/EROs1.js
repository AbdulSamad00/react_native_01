        ...
        const filterEROs = data.map((ero) => {
          return {
           ...
           // BirthDate: dayjs(ero?.user?.dateBirth).format("DD-MM-YYYY"),
            ...
            "Branch of Bank": ero?.user?.bankInfo?.branchOfBank,
            "Prim InsuranceNo": ero?.user?.insurance?.primInsuranceNo,
            "Prim Insurance": ero?.user?.insurance?.primInsurance,
            "Prim Insurance ValidTill":ero?.user?.insurance?.primInsuranceValidTill,
            "Sec. InsuranceNo": ero?.user?.insurance?.secInsuranceNo,
            "Sec. Insurance": ero?.user?.insurance?.secInsurance,
            "Sec. Insurance ValidTill": ero?.user?.insurance?.secInsuranceValidTill,
            ...
            "IDPaper ValidTill": ero?.user?.identification?.idPaperValidTill,
          };
        });
        let wrap = {};
        const comp = (item) => <Text style={{ color: "black" }}>{item}</Text>;
        Object.keys(filterEROs[0]).forEach((key) => {
          wrap[key] = { cellWidth: 100, component: comp };
        });
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        ...
        <IconButton
          ...
          onPress={() => {
            ...
            checkedEROs.forEach(async (eroId) => {
              const { ok } = await deleteERO(eroId);
              if (ok) {
                console.log("deleted");
              }
            });
